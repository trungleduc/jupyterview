...
export class MockModel {
  ...
  private _callbacks: { [key: string]: Array<() => void> };
  constructor() {
    this._value = {};
    this._callbacks = {};
  }

  public listenTo(object: any, events: string, callback: () => void) {
    if (this._callbacks[events]) {
      this._callbacks[events].push(callback);
    } else {
      this._callbacks[events] = [callback];
    }
  }
